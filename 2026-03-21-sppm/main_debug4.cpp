...
void photonPass(const Scene& sc, RNG& rng, int N,
    ...
    // 朗伯面光源向下半球：总功率 = L_emit * π * A
    // 每个光子代表 L_emit * π * A / N，但余弦加权采样时 PDF = cos/π
    // 每个光子发射：importance weight = cos(θ) / (cos(θ)/π) = π
    // 所以 photonPower = L_emit * A / N（已包含 π 归一化）
...
void sppmUpdate(std::vector<HitPoint>& hps,
    ...
    for (auto& hp : hps) {
        ...
        hp.radius2 *= ratio;
        // accFlux = weight * albedo * dFlux（累积，通过 ratio 维持一致性）
        hp.accFlux = (hp.accFlux + hp.weight * hp.albedo * dFlux) * ratio;
        hp.N += (int)(alpha * M);
    }
}
...
int main() {
    ...
    // DEBUG
    {
        double sum=0, maxF=0; int cnt=0;
        double sumR2=0;
        for (auto& hp : hps) {
            if (hp.valid && hp.N > 0) {
                double f = hp.accFlux.x+hp.accFlux.y+hp.accFlux.z;
                sum += f; maxF = std::max(maxF,f); cnt++;
                sumR2 += hp.radius2;
            }
        }
        if (cnt>0) {
            double avgR2 = sumR2/cnt;
            double avgF = sum/cnt;
            printf("DEBUG: N_with_photons=%d, avg_accFlux=%.6e, max_accFlux=%.6e\n",
                   cnt, avgF, maxF);
            printf("DEBUG: avg_radius2=%.6e, totalPhotons=%lld\n",
                   avgR2, totalPhotons);
            double L_sample = maxF/3.0 / (PI * avgR2 * totalPhotons);
            printf("DEBUG: sample L (max/3 chan / PI/r2/N) = %.6e\n", L_sample);
        }
        fflush(stdout);
    }
    // ------ Reconstruct ------
