...
Material makeWax() {
    // 蜡烛：高散射，强橙色透射（σ_a 降低 = 吸收少 = 更容易穿透）
    return { MAT_SSS_WAX,
             Vec3(0.95, 0.85, 0.70),  // 表面：奶白色
             Vec3(1.0, 0.55, 0.1),    // SSS：深橙色（更饱和）
             9.0, 0.2, 0.3, 0.6, 0.0 };
}
Material makeSkin() {
    // 皮肤：中等散射，明显红色透射
    return { MAT_SSS_SKIN,
             Vec3(0.9, 0.7, 0.6),     // 表面
             Vec3(1.0, 0.35, 0.25),   // SSS：鲜红色（模拟血液）
             6.0, 0.4, 0.1, 0.7, 0.0 };
}
Material makeJade() {
    // 玉石：低散射，翠绿透射
...
Vec3 computeSSS(const Vec3& hitPoint, const Vec3& normal,
    ...
    Vec3 sssContrib = (backlight + frontSSS) * atten;
    return clamp(sssContrib, 0.0, 3.0);
}
...
Vec3 ambientLight(const Vec3& normal, const Vec3& albedo) {
    ...
    Vec3 skyColor(0.4, 0.55, 0.8);
    Vec3 groundColor(0.12, 0.10, 0.08);
    Vec3 ambient = lerp(groundColor, skyColor, skyFactor);
    return albedo * ambient * 0.12;   // 降低到0.12，让直接光/SSS更突出
}
...
void buildScene(std::vector<Sphere>& spheres, std::vector<Light>& lights) {
    ...
    // 背光 1（正后方强蓝光，专门激发 SSS 透射效果）
    Light backLight;
    backLight.pos = Vec3(0.0, 0.5, -9.0);
    backLight.color = Vec3(0.4, 0.6, 1.0);
    ...
    backLight2.color = Vec3(1.0, 0.7, 0.3);
    backLight2.intensity = 2.2;
    lights.push_back(backLight2);

    // 底部补光（地面反弹，弱）
    Light fillLight;
    fillLight.pos = Vec3(0.0, -2.5, -1.0);
    fillLight.color = Vec3(0.6, 0.5, 0.4);
...
int main() {
    ...
    const int SAMPLES = 8; // 提高采样数，减少噪点
