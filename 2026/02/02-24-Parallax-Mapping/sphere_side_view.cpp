...
Vec2 parallax_mapping(const Vec2& tex_coords, const Vec3& view_dir_tangent) {
    const double height_scale = 0.15;  // 增大到0.15
...
void render_scene(const std::string& filename, bool use_parallax, const std::string& description,
    ...
    Sphere sphere(Vec3(0, 0, 0), 1.0);  // 球心在原点
    ...
    for (int j = 0; j < HEIGHT; j++) {
        ...
        for (int i = 0; i < WIDTH; i++) {
            ...
            // 从相机位置向球心方向看，但保持正交投影的射线方向
            Vec3 look_dir = (Vec3(0, 0, 0) - camera_pos).normalize();
            Vec3 right = Vec3(0, 1, 0).cross(look_dir).normalize();
            Vec3 up = look_dir.cross(right).normalize();
            
            Vec3 ray_dir = (look_dir + right * x + up * y).normalize();
...
int main() {
    std::cout << "========================================" << std::endl;
    std::cout << "  Parallax Mapping - 侧视角测试" << std::endl;
    std::cout << "========================================" << std::endl;
    
    // 从侧面观察球体（相机在X=2, Y=1, Z=3的位置）
    Vec3 camera_pos(2.0, 1.0, 3.0);
    
    render_scene("sphere_side_normal.ppm", false, "渲染图1：普通纹理（侧视角）", camera_pos);
    render_scene("sphere_side_parallax.ppm", true, "渲染图2：视差贴图（侧视角）", camera_pos);
