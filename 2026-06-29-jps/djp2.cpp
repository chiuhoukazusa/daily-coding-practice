...
void successors(const G& g, Pt par, Pt cur, Pt t, vector<Succ>& out) {
    ...
    if(dx!=0&&dy!=0){
        Pt jp;if((jp=jump(g,cur,dx,dy,t)).x!=-1)out.push_back({jp,max(abs(jp.x-cur.x),abs(jp.y-cur.y))*14});
        ...
        Pt jp;if((jp=jump(g,cur,dx,0,t)).x!=-1)out.push_back({jp,abs(jp.x-cur.x)*10});
        if(!g.op(cur.x,cur.y-1)&&g.op(cur.x-dx,cur.y-1))if((jp=jump(g,cur,dx,-1,t)).x!=-1)out.push_back({jp,14});
        if(!g.op(cur.x,cur.y+1)&&g.op(cur.x-dx,cur.y+1))if((jp=jump(g,cur,dx,1,t)).x!=-1)out.push_back({jp,14});
    }else{
        Pt jp;if((jp=jump(g,cur,0,dy,t)).x!=-1)out.push_back({jp,abs(jp.y-cur.y)*10});
        if(!g.op(cur.x-1,cur.y)&&g.op(cur.x-1,cur.y-dy))if((jp=jump(g,cur,-1,dy,t)).x!=-1)out.push_back({jp,14});
        if(!g.op(cur.x+1,cur.y)&&g.op(cur.x+1,cur.y-dy))if((jp=jump(g,cur,1,dy,t)).x!=-1)out.push_back({jp,14});
    }
}
...
vector<Pt> jps_path(const G& g, Pt s, Pt t, int& cost_out) {
    ...
    unordered_map<Pt,N,PH> m;auto cmp=[](const N&a,const N&b){return a.f>b.f;};
    ...
    while(!q.empty()){
        ...
        if(c.p==t){vector<Pt> path;for(Pt p=t;p.x!=-1;p=m[p].par)path.push_back(p);reverse(path.begin(),path.end());cost_out=c.g;return path;}
        vector<Succ> succs;
        ...
        for(auto& sc:succs){int ng=c.g+sc.cost;auto it=m.find(sc.p);
            if(it==m.end()||ng<it->second.g){m[sc.p]={ng+heur(sc.p,t),ng,sc.p,c.par.closed?c.p:sc.p,false};q.push(m[sc.p]);}}
    }
    cost_out=-1;return{};
}
...
int main(){
    G g=maze(64,64,42);Pt s{0,0},t{63,63};
    int jps_cost;vector<Pt> path=jps_path(g,s,t,jps_cost);
    printf("JPS reported cost: %d, path size: %zu\n",jps_cost,path.size());
    // Manual cost
    int mc=0;
    for(size_t i=0;i+1<path.size();i++){
        Pt a=path[i],b=path[i+1];int dx=abs(b.x-a.x),dy=abs(b.y-a.y);
        int diag=min(dx,dy),straight=(dx>dy?dx-dy:dy-dx);
        int seg=diag*14+straight*10;mc+=seg;
        // printf("  (%d,%d)->(%d,%d): cost=%d cum=%d\n",a.x,a.y,b.x,b.y,seg,mc);
    }
    printf("Manual cost: %d\n",mc);
    printf("Delta: %d\n",jps_cost-mc);
    // Check if JPS cost matches manual if we use the SAME formula as successors
    // The successors use max(|dx|,|dy|) * 14 for diagonal
    int mc2=0;
    for(size_t i=0;i+1<path.size();i++){
        Pt a=path[i],b=path[i+1];
        int dx=b.x-a.x,dy=b.y-a.y;
        int cost;
        if(dx!=0&&dy!=0)cost=max(abs(dx),abs(dy))*14;
        else cost=(abs(dx)+abs(dy))*10;
        mc2+=cost;
    }
    printf("Manual with JPS formula: %d\n",mc2);
    return 0;
}
