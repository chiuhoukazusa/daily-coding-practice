...
int main() {
    ...
    // 几个彩色立方体（更平衡的颜色）
    auto cube1 = createCube(Vec3(0, 0, 0), 1.5, Vec3(0.8, 0.2, 0.2));      // 红色
    auto cube2 = createCube(Vec3(-2, 0, -1), 1.0, Vec3(0.2, 0.8, 0.2));    // 绿色
    auto cube3 = createCube(Vec3(2, 1, 1), 0.8, Vec3(0.2, 0.2, 0.8));      // 蓝色
    auto cube4 = createCube(Vec3(0, -2, 0), 3.0, Vec3(0.5, 0.5, 0.5));     // 灰色地板
    auto cube5 = createCube(Vec3(1, -0.5, -2), 0.6, Vec3(0.9, 0.7, 0.2));  // 黄色
    auto cube6 = createCube(Vec3(-1.5, 0.5, 1), 0.7, Vec3(0.8, 0.2, 0.8)); // 紫色
    ...
    scene.insert(scene.end(), cube5.begin(), cube5.end());
    scene.insert(scene.end(), cube6.begin(), cube6.end());
    ...
    rasterizer.save("rasterizer_colorful.png");
