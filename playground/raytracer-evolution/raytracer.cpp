...
struct Sphere {
    ...
    Sphere(Point3 c, double r, Material* m) : center(c), radius(r), material(m) {}
...
Color rayColor(const Ray& r, const Scene& scene, int depth) {
    ...
    if (scene.hit(r, 0.001, INF, rec)) {
        Ray scattered(Point3(0,0,0), Vec3(0,0,1));  // 初始化
        Color attenuation;
...
int main() {
    ...
    // 材质
    Material groundMaterial(LAMBERTIAN, Color(0.8, 0.8, 0.0), 0, 0);
    Material centerMaterial(LAMBERTIAN, Color(0.1, 0.2, 0.5), 0, 0);
    Material leftMaterial(DIELECTRIC, Color(1, 1, 1), 0, 1.5);
    Material rightMaterial(METAL, Color(0.8, 0.6, 0.2), 0.0, 0);
    
    // 场景
    Scene scene;
    scene.spheres.push_back(Sphere(Point3(0, -100.5, -1), 100, &groundMaterial));
    scene.spheres.push_back(Sphere(Point3(0, 0, -1), 0.5, &centerMaterial));
    scene.spheres.push_back(Sphere(Point3(-1, 0, -1), 0.5, &leftMaterial));
    scene.spheres.push_back(Sphere(Point3(1, 0, -1), 0.5, &rightMaterial));
    
    // 相机
    Camera camera(Point3(0, 0, 0), Point3(0, 0, -1), Vec3(0, 1, 0), 90, aspectRatio);
    
    // 渲染
    render("phase2_materials.png", imageWidth, imageHeight, samplesPerPixel, maxDepth, scene, camera);
